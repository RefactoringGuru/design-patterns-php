 ...
 * только для кошек. Каждую запись в базе данных представляет объект Кошка. Его
 * данные состоят из двух частей:
 *
 * 1. Уникальные (внешние) данные, такие как имя питомца, возраст и информация о
 * владельце.
 * 2. Общие данные (внутренние), такие как название породы, цвет, текстура и
 * т.д.
 *
 * Первая часть хранится непосредственно внутри класса Кошка, который играет
 * роль контекста. Вторая часть, однако, хранится отдельно и может совместно
 * использоваться разными кошками. Эти совместно используемые данные находятся
 * внутри класса РазновидностиКошек. Все кошки, имеющие схожие признаки,
 * привязаны к одному и тому же классу РазновидностейКошек, вместо того чтобы
 * хранить повторяющиеся данные в каждом из своих объектов.
 */
